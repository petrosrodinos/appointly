...
const PhoneNumber = React.forwardRef<HTMLInputElement, PhoneNumberProps>(({ className, value = "", countryCode, onValueChange, onCountryCodeChange, defaultCountryCode = "+30", disabled, ...props }, ref) => {
  ...
  return (
    <div className={cn("flex w-full items-center rounded-md border border-input bg-background overflow-hidden", disabled && "opacity-50 cursor-not-allowed", className)}>
      ...
      <Input ref={ref} type="tel" value={value} onChange={handlePhoneChange} disabled={disabled} {...props} className={cn("h-11 flex-1 rounded-none bg-background px-3", "border-none !border-0 !outline-none !shadow-none", "focus-visible:!ring-0 focus-visible:!ring-offset-0", "focus:!ring-0 focus:!ring-offset-0", "[&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none")} />
    </div>
  );
});
